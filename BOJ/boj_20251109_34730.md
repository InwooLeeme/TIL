...
정석적인 다익스트라 + 정점 분할 문제고 그냥 **잘**구현해주면 된다.
...
void solve(){
	ri(N, M, K, S, T);
	for(int i = 0; i < M; i++){
		int a,b,c,o; ri(a, b, c, o);
		g[a].pb({c, o, b});
	}
	vvi dist(N + 1, vi(K + 1, INF));
	minpq<ti3> pq;
	pq.push({dist[S][0] = 0, 0, S});
	while(si(pq)){
		auto [cdist, ocur, cur] = pq.top(); pq.pop();
		if(dist[cur][ocur] < cdist) continue;
		if(cur == T){
			po(cdist);
			return;
		}
		for(const auto& [cost, onxt, nxt] : g[cur]){
			if(cdist % onxt != 0) continue;
			if(dist[nxt][onxt] > cdist + cost){
				dist[nxt][onxt] = cdist + cost;
				pq.push({dist[nxt][onxt], onxt, nxt});
			}
		}
	}
	po(-1);
